...
class DH:
    ...
    Key:
    DHSender and DHReceiver agree on a cyclic group
    In this case, we pass an AbGroup object, which holds the parameters of
    an abelian group along with a group element group.g and its order group.q
    With this, a cyclic group of order group.q generated by group.g is agreed upon
...
class DHSender(DH):
    ...
    ----------
    msg_to_send: bytes
        Message DHSender plans on sending
    a: int
        A random integer between 1 and group.q, the order of group.g
    encode: AESCode
...
class DHAttacker(DH):
    ...
    def _prep_kangaroo(self):
        ...
        end = (self.group.q - 1) // self.part_key_mod + 1
...
class Server:
    def __init__(self, email: bytes, password: bytes,
        ...
        self.k = k
        self.E = email
        self.salt = rand_bytes(8)  # Generate a 64-bit salt
